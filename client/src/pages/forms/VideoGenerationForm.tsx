...
export default function VideoGenerationForm() {
  ...
  const template = getTemplateById('video-generation');
  if (!template) return <div>Template not found</div>;

  const { mutate: generatePrompt, isPending: isGenerating } = useMutation({
    mutationFn: async (data: Record<string, string>) => {
      return await apiRequest('/api/generate-video-prompt', 'POST', {
        ...data,
        role: selectedRole,
        goal: selectedGoal,
        description
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8 flex items-center justify-between">
          ...
          <h2 className="text-3xl font-bold text-gray-900">Video Generation Prompt Producer</h2>
          <p className="text-gray-600 mt-2">Create prompts for AI video generators like Runway, Pika Labs, and Stable Video Diffusion.</p>
        </div>
      </div>
